...
public class Car {
    ...
    public String checkCar(){
        ...
        if (!engine.workingEngine()) {  // Assuming `Engine` class has `isWorkingProperly` method
            return "The engine is not working properly!";
        }

        // Check if the seal is eligible to drive
        if (!driver.isEligibleDriver()) {  // Assuming `Seal` class has `isEligibleDriver` method
            return "The seal is not allowed to drive a car!";
        }

        // If all checks pass, car is ready
